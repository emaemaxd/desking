...
public class TimerecordingRepository implements PanacheRepository<Timerecording> {
    ...
    public List getEntriesForProject(int id){
        var query = getEntityManager().createQuery("select pe.id.entry,p,pe.id.entry.user from ProjectEntries pe join Projects p on p.id = pe.id.project.id where pe.id.project.id = :id");

    ...
    return (
        ...
        <div className={`${styles.wrapper} ${props.hideMenu && styles.hideInner}`}>
            ...
            <div className={styles.links}>
                <Link className={props.current === "/arbeitszeiten" && styles.active}
                      to="/arbeitszeiten">Arbeitszeiten</Link>
                <Link className={props.current === "/urlaubsantraege" && styles.active}
                      to="/urlaubsantraege">Urlaubsanträge</Link>
            ...
            <div className={styles.icons}>
                <Link className={`${styles.userWrapper} ${props.current === "/profil" && styles.active}`} to="/profil">
                    <img style={{width: 35, height: 35, marginLeft: 15, marginRight: 15}} alt="Profil"
                         src={props.current === "/profil" ? UserBlack : User}/>
                </Link>
                <Link to="/login">
                    <img className={styles.hideOnMobile} style={{width: 25}} alt="Logout" src={Logout}/>
                </Link>
                <div role="button" tabindex="0" onClick={e => setBurgerOpen(!burgerOpen)}
                     onKeyDown={e => console.log(e)}>
                    <img className={styles.hideOnDesktop} style={{width: 25}} alt="Burger Menu"
                         src={burgerOpen ? BurgerOpen : BurgerClosed}/>
                </div>
            </div>
